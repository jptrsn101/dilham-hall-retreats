...
- **Pages:** `index.html` (home), `stay.html` (hub: three-way chooser, comparison table, all-properties calendar, Canal Camping section), `broad-fen.html` and `tonnage-bridge.html` (brand landing pages, one per retreat, each with its own SuperControl calendar, FAQ + FAQPage schema; these are the URLs to share on social, added Sept 2026), `pod-damselfly.html` / `pod-dragonfly.html` / `pod-swallowtail.html` (Broad Fen pods), `tonnage-king-twin.html` / `tonnage-king-single-bunk.html` / `tonnage-king-bunk.html` (Tonnage styles), `group-bookings.html`, `canoe-hire.html`, `events.html` (with enquiry form), `things-to-do.html` (categorised, image slots for owner to fill), `about.html`, `reviews.html`, `contact.html`, plus terms/privacy/CCTV/thanks/404.
- **Brand URLs:** `/broad-fen` and `/tonnage-bridge` are the canonical pages for each retreat. Old `/stay#couples` and `/stay#tonnage` anchors are redirected by JS in `main.js`. `tonnagebridge.co.uk` 301s to `/tonnage-bridge`.
